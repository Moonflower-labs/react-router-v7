import { useParams, Link, useRouteError, data, useRouteLoaderData } from "react-router";
...
export default function SoulDetail({ loaderData }: Route.ComponentProps) {
...
export function ErrorBoundary() {
  ...
  const { data } = useRouteError() as { data: { error: string } };

  return (
    <div className="text-3xl flex h-screen justify-center items-center">
      <div className="text-center">
        {data?.error ? <p className="mb-4">{data.error}</p> : <p className="mb-4"> No hemos encontrado el video 😩 con ID {params.id}.</p>}
