    ...
    <div className="text-center pt-4 px-1">
      ...
      {sessions.length > 0 ? sessions.map((session) => (
        <div key={session.id} className="p-2 rounded-lg shadow-sm max-w-3xl mx-auto mb-4 border border-base-300">
          <h2 className="text-xl font-bold flex justify-between px-4"><span>{session.name}</span> <span>{new Date(session.startDate).toLocaleDateString()}</span></h2>
          <p className="p-4">{session.description}</p>
          <p className="mb-2">Comienza: {new Date(session.startDate).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</p>
          <p className="mb-2">Finaliza: {new Date(session.endDate).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</p>
