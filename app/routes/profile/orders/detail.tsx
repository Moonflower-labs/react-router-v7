        ...
        <div className="mb-6 lg:w-2/3 mx-auto px-4">
            {!order ? (
                <div className="text-center mx-auto pt-20">
                    <div>No hemos encontrado el pedido {params.orderId}</div>
                    <Link to={href("/profile/orders")} className="link link-primary">Volver a Pedidos</Link>
                </div>
            ) : (<>
                <h2 className="text-xl text-primary text-center mx-auto my-5">
                    Pedido realizado el <span className="font-bold">{formatDayTimeEs(order?.createdAt as Date)}</span>
                </h2>
                {order?.guest
